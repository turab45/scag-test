#!/usr/bin/env python3
"""
coactivation_analysis_9concepts_fullimg.py
===========================================

Cardiac DenseNet-161 coactivation study: build + analyse.
**9-concept (derived AHA-style LA) FULL-IMAGE variant.**
...
concepts (LV/MYO/RV x basal/mid/apical) on the FULL (uncropped) image. It uses
the full-image dataset module exactly as in your 9-concept full-image notebook --

    from MnMs2DatasetConcpets import MnMsDatasetLAX
    MnMsDatasetLAX(root, max_images=None)         # default -> 9 derived LA concepts

Note: this module returns the 9 derived concepts BY DEFAULT (no flag). Passing
get_original_concepts=True would instead return the 3 GT concepts (that is the
3-concept full-image variant). The cropped variants use
MnMs2DatasetConcpetsHeartUnion(... crop_to_heart_union=True) instead.
...
The only differences from the cropped script are:
  ...
  * cfg.concepts lists the 9 derived names,
  * cfg.image_tag = 'fullimg', so graph/npz filenames carry "fullimg" and never
    collide with the cropped or 3-concept artifacts.
...
def build_graphs_and_npz(cfg):
    ...
    log("loading MnM2 LAX dataset (full image, default -> 9 derived LA concepts) ...")
...
def main():
    ...
    cfg.out_dir = os.path.join('results_mnm2', f'analysis_9c_fullimg_B{cfg.block}')
    ...
    # ---- the nine derived AHA-style LA concepts on the FULL (uncropped) image ----
    ...
    cfg.qualitative_graphs = ['spatial', 'pooled']
