#!/usr/bin/env python3
"""
Concepts three with heart cropped region.
"""
...
def build_graphs_and_npz(cfg):
    ...
    from my_coactivation_graph import CoactivationGraphBuilder
    from MnMs2DatasetConcpetsHeartUnion import MnMsDatasetLAX
    ...
    log("loading MnM2 LAX dataset ...")
    full_dataset = MnMsDatasetLAX(cfg.mnm2_root, max_images=None,
                                  include_heart_union_only=False, crop_to_heart_union=True)
...
def pooled_graph_name(cfg):
    return f"coactivation_3_dice0.06_cropped_{float(cfg.threshold)}_B{cfg.block}.graphml"


def spatial_graph_name(cfg):
    return f"coactivation_3_dice0.06_cropped_{float(cfg.threshold)}_B{cfg.block}_SPATIAL.graphml"


def npz_name(cfg):
    return f"3_cropped_dice0.06_B{cfg.block}_concept_associations_SPATIAL.npz"
...
def node_substructures(nodes, eu, ev, agree, hard, node_score, node_thresh, min_size=3):
    """Agreement edges restricted to CORE nodes (node-homophily >= node_thresh)."""
...
def substructure_report(subs, hard, n_nodes, concept_names, top=8):
    ...
    for sid, s in enumerate(subs):
        ...
        table.append(dict(id=sid, size=s['size'],
                          ...
                          spatial_dice=s.get('spatial_dice', float('nan')),
                          spatial_corr=s.get('spatial_corr', float('nan')),
                          peak_agreement=s.get('peak_agreement', float('nan')),
                          validation_images=s.get('validation_images', 0),
                          validation_pairs=s.get('validation_pairs', 0),
...
def _sample_pairs(n_items, max_pairs, rng):
    n_all = n_items * (n_items - 1) // 2
    if n_all <= max_pairs:
        return np.triu_indices(n_items, k=1)
    pairs = set()
    while len(pairs) < max_pairs:
        a = int(rng.integers(0, n_items))
        b = int(rng.integers(0, n_items - 1))
        if b >= a:
            b += 1
        if a > b:
            a, b = b, a
...
def validate_spatial_alignment(subs, nidx, spatial_maps, cfg):
    """Validate whether substructure neurons activate overlapping spatial cells.

    spatial_maps is expected to be (N_images, N_neurons, N_cells), where N_cells
    is the flattened feature-map grid from the analysed DenseNet layer.
    """
    if spatial_maps is None or not subs:
        return dict(spatial_dice_mean=float('nan'), spatial_corr_mean=float('nan'),
                    peak_agreement_mean=float('nan'), spatial_validated_subs=0)
    if spatial_maps.ndim != 3:
        raise ValueError(f"expected spatial_maps with 3 dims, got {spatial_maps.shape}")
    if spatial_maps.shape[1] <= int(nidx.max()):
        raise ValueError("spatial_maps neuron dimension does not cover graph neuron indices")

    rng = np.random.default_rng(cfg.seed)
    image_ids = np.arange(spatial_maps.shape[0])
    if cfg.validation_max_images and len(image_ids) > cfg.validation_max_images:
        image_ids = rng.choice(image_ids, cfg.validation_max_images, replace=False)
        image_ids.sort()

    dice_scores, corr_scores, peak_scores = [], [], []
    for sid, sub in enumerate(subs):
        ...
        pu, pv = _sample_pairs(len(neuron_ids), cfg.validation_max_pairs, rng)
        image_dice, image_corr, image_peak = [], [], []
        for img_id in image_ids:
            maps = np.asarray(spatial_maps[img_id, neuron_ids, :], dtype=np.float64)
            if not np.isfinite(maps).all():
                continue

            thr = np.quantile(maps, cfg.validation_activation_quantile,
                              axis=1, keepdims=True)
            active = maps >= thr
            active_count = active.sum(axis=1).astype(np.float64)
            inter = np.logical_and(active[pu], active[pv]).sum(axis=1).astype(np.float64)
            denom = active_count[pu] + active_count[pv]
            valid = denom > 0
            if valid.any():
                image_dice.append(float(np.mean((2.0 * inter[valid]) / denom[valid])))

            centered = maps - maps.mean(axis=1, keepdims=True)
            norm = np.linalg.norm(centered, axis=1)
            denom_corr = norm[pu] * norm[pv]
            valid_corr = denom_corr > 1e-12
            if valid_corr.any():
                corr = (centered[pu] * centered[pv]).sum(axis=1)
                image_corr.append(float(np.mean(corr[valid_corr] / denom_corr[valid_corr])))

            peaks = np.argmax(maps, axis=1)
            _, counts = np.unique(peaks, return_counts=True)
            image_peak.append(float(counts.max() / len(peaks)))

        if not image_dice:
            continue
        sub['spatial_dice'] = round(float(np.mean(image_dice)), 4)
        sub['spatial_corr'] = round(float(np.mean(image_corr)), 4) if image_corr else float('nan')
        sub['peak_agreement'] = round(float(np.mean(image_peak)), 4) if image_peak else float('nan')
        sub['validation_images'] = int(len(image_dice))
        sub['validation_pairs'] = int(len(pu))
        dice_scores.append(sub['spatial_dice'])
        if np.isfinite(sub['spatial_corr']):
            corr_scores.append(sub['spatial_corr'])
        if np.isfinite(sub['peak_agreement']):
            peak_scores.append(sub['peak_agreement'])

    return dict(
        spatial_dice_mean=round(float(np.mean(dice_scores)), 4) if dice_scores else float('nan'),
        spatial_corr_mean=round(float(np.mean(corr_scores)), 4) if corr_scores else float('nan'),
        peak_agreement_mean=round(float(np.mean(peak_scores)), 4) if peak_scores else float('nan'),
        spatial_validated_subs=len(dice_scores))
...
def analyse_graph(graph_path, assoc, concept_names, spatial_maps, visual_inputs, cfg, tag):
    ...
    for mode in ['hard', 'jsd']:
        ...
        for kind, subs in [('edge', edge_subs), ('node', node_subs), ('combined', comb_subs)]:
            spatial_agg = {}
            if cfg.validate_spatial_alignment and spatial_maps is not None:
                spatial_agg = validate_spatial_alignment(subs, nidx, spatial_maps, cfg)
            agg, top_tbl, full_tbl = substructure_report(subs, hard, n_nodes,
                                                         concept_names, top=8)
            agg.update(spatial_agg)
            reports[kind] = dict(agg=agg, table=full_tbl, _subs=subs)
            if (cfg.visualize_spatial_evidence and mode in cfg.visual_modes
                    and kind in cfg.visual_kinds):
                plot_spatial_evidence(subs, nidx, spatial_maps, visual_inputs,
                                      concept_names, tag, mode, kind, cfg)
        ...
        for kind in ['edge', 'node', 'combined']:
            ...
            if 'spatial_dice_mean' in agg:
                print(f"     spatial validation: dice={agg['spatial_dice_mean']} "
                      f"corr={agg['spatial_corr_mean']} "
                      f"peak_agreement={agg['peak_agreement_mean']} "
                      f"validated_subs={agg['spatial_validated_subs']}")
    ...
    for r in results:
        for mode, rec in r['local'].items():
            ...
            for kind in ['edge', 'node', 'combined']:
                ...
                for metric in ['spatial_dice_mean', 'spatial_corr_mean',
                               'peak_agreement_mean', 'spatial_validated_subs']:
                    if metric in agg:
                        row[f"{kind}_{metric}"] = agg[metric]
            flat.append(row)
            ...
            for kind in ['edge', 'node', 'combined']:
                for t in rec['substructures'][kind]['table']:
                    detail.append(dict(graph=r['tag'], labelling=mode, type=kind,
                                       ...
                                       entropy=t['entropy'],
                                       spatial_dice=t.get('spatial_dice', float('nan')),
                                       spatial_corr=t.get('spatial_corr', float('nan')),
                                       peak_agreement=t.get('peak_agreement', float('nan')),
                                       validation_images=t.get('validation_images', 0),
                                       validation_pairs=t.get('validation_pairs', 0),
            ...
            entry['local'][mode] = dict(
                ...
                substructures={kind: dict(
                    ...
                    members=[dict(
                        ...
                        spatial_corr=s.get('spatial_corr', float('nan')),
                        peak_agreement=s.get('peak_agreement', float('nan')))
                        for s in rec['substructures'][kind]['_subs'][:50]])
                    for kind in ['edge', 'node', 'combined']})
        serial.append(entry)
...
def main():
    ...
    # ---- what to run ----
    cfg.build = True             # set True to rebuild model activations/graphs
    cfg.save_spatial_maps = True   # needed for post-hoc substructure activation validation
    cfg.save_visual_evidence_inputs = True  # saves MRI tensors + IDs for overlay plots
    ...
    cfg.out_dir = os.path.join('results_mnm2', f'analysis_3c_cropped_B{cfg.block}')
    ...
    cfg.seed = 0
    cfg.validate_spatial_alignment = True
    cfg.validation_activation_quantile = 0.95  # top 5% feature-map cells are "active"
    cfg.validation_max_images = 128            # 0/None = use all images
    cfg.validation_max_pairs = 2000            # sampled neuron pairs per substructure
    cfg.visualize_spatial_evidence = False
    cfg.visual_modes = ['hard', 'jsd']                # add 'jsd' if you want both
    cfg.visual_kinds = ['edge', 'node', 'combined']            # e.g. ['edge', 'node', 'combined']
    cfg.visual_top_subs = 10
    cfg.visual_top_images = 4
    cfg.visual_top_neurons = 6
    cfg.visual_activation_quantile = 0.95
    ...
    cfg.qualitative_graphs = ['pooled', 'spatial']
    cfg.qualitative_modes = ['hard', 'jsd']
    cfg.qualitative_kinds = ['node', 'edge', 'combined']
    ...
    cfg.qualitative_activation_quantile = 0.95
    ...
    cfg.plot = False              # draw substructures on the graph
    cfg.plot_modes = ['hard', 'jsd']     # which labelling modes to plot (add 'jsd' if wanted)
    ...
    log(f"config: build={cfg.build} qualitative={cfg.qualitative} gpu_spearman={cfg.use_gpu_spearman} "
    ...
    for pth in (pooled_path, spatial_path, npz_path):
    ...
    if spatial_maps is None:
        log("spatial activation maps not found in .npz; substructure spatial validation skipped "
            "(set cfg.build=True once to save them)")
    else:
        log(f"spatial activation maps {spatial_maps.shape}; validation enabled={cfg.validate_spatial_alignment}")
    if visual_inputs.get('sample_images') is None or visual_inputs.get('concept_masks') is None:
        log("sample MRI images or GT concept masks not found in .npz; visual evidence plots skipped "
            "(set cfg.build=True once to save them)")
    if cfg.qualitative:
    ...
    results = [analyse_graph(pooled_path, assoc, concept_names, spatial_maps, visual_inputs, cfg, 'pooled'),
               analyse_graph(spatial_path, assoc, concept_names, spatial_maps, visual_inputs, cfg, 'spatial')]
