#!/usr/bin/env python3
"""
coactivation_analysis_9concepts.py
==================================

Cardiac DenseNet-161 coactivation study: build + analyse.
**9-concept (derived AHA-style LA) variant.**

This is the SAME pipeline as the 3-concept `coactivation_analysis.py`, but the
neuron-concept association (NCA) matrix is built over the nine derived LA
concepts produced by the dataset's `derive_lax_concepts=True` path:
...
on-graph substructure plots -- is identical and already concept-count-agnostic
(it reads concept_names from the npz and uses rows.shape[1] throughout).

All configuration lives in main() -- edit the variables there. No CLI args.
The only functional differences from the 3-concept script are:
  * cfg.concepts now lists the 9 derived names,
  * the dataset is constructed with derive_lax_concepts=True,
  * graph/npz filenames are prefixed by len(cfg.concepts) (-> "9_") so the
    3- and 9-concept artifacts never collide.
...
def build_graphs_and_npz(cfg):
    ...
    log("loading MnM2 LAX dataset (derive_lax_concepts=True) ...")
...
def pooled_graph_name(cfg):
    nc = len(cfg.concepts)
    return f"coactivation_{nc}_dice0.06_cropped_{float(cfg.threshold)}_B{cfg.block}.graphml"


def spatial_graph_name(cfg):
    nc = len(cfg.concepts)
    return f"coactivation_{nc}_dice0.06_cropped_{float(cfg.threshold)}_B{cfg.block}_SPATIAL.graphml"


def npz_name(cfg):
    nc = len(cfg.concepts)
    return f"{nc}_cropped_dice0.06_B{cfg.block}_concept_associations_SPATIAL.npz"
...
def main():
    ...
    # ---- what to run ----
    cfg.build = True              # one-time rebuild saves tensors for qualitative figures
    ...
    cfg.out_dir = os.path.join('results_mnm2', f'analysis_9c_cropped_B{cfg.block}')
    ...
    # ---- the nine derived AHA-style LA concepts (from derive_lax_concepts=True) ----
    cfg.concepts = ['LV_basal', 'LV_mid', 'LV_apical',
                    'MYO_basal', 'MYO_mid', 'MYO_apical',
                    'RV_basal', 'RV_mid', 'RV_apical']
    ...
    cfg.qualitative_graphs = ['pooled']
    cfg.qualitative_modes = ['jsd', 'hard']
    cfg.qualitative_kinds = ['node', 'combined', 'edge']
